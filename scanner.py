...
def main():
    ...
    nifty_price = round(float(nifty_close.iloc[-1]), 2)
    ...
    for stock in stocks:
        result = score_stock(stock, nifty_return)
    ...
    timestamp = india_now.strftime("%d-%m-%Y %I:%M:%S %p IST")
    ...
    if len(top5) == 0:
        ...
        for i, s in enumerate(top5, start=1):
            ...
            entry = s["price"]
            sl = round(entry - s["atr"], 2)
            target = round(entry + (2 * s["atr"]), 2)
